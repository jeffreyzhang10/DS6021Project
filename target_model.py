...
def build_offense_vectors(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    start_cols = ["game_id", "play_id", "nfl_id", "x", "y", "s", "a"]
    ...
    final["dist_to_ball"] = np.sqrt(
        (final["x"] - final["ball_land_x"]) ** 2
        + (final["y"] - final["ball_land_y"]) ** 2
    )
    final["dx"] = final["x"] - final["x_start"]
    final["dy"] = final["y"] - final["y_start"]
    ...
    if not def_final.empty and not off.empty:
        pairs = off.merge(
            def_final[["game_id", "play_id", "nfl_id", "x", "y"]],
            on=["game_id", "play_id"],
            suffixes=("_off", "_def"),
        )
    ...
    if not qb_final.empty:
        ...
        off["vec_p_to_qb_x"] = off["qb_x_final"] - off["x"]
        off["vec_p_to_qb_y"] = off["qb_y_final"] - off["y"]
